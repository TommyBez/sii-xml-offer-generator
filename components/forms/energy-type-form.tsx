...
export function EnergyTypeForm({ onSubmit: _onSubmit }: EnergyTypeFormProps) {
  ...
  // Watch field values for validation status and store updates
  const marketType = form.watch('energyType.TIPO_MERCATO');
  const includeGreenOptions = form.watch('energyType.INCLUDE_GREEN_OPTIONS');
  const actionType = form.watch('energyType.AZIONE');
  ...
  useEffect(() => {
    if (marketType || includeGreenOptions !== undefined || actionType) {
      // Update offerDetails.TIPO_MERCATO in store as required
      updateFormData('offerDetails', { TIPO_MERCATO: marketType });
      
      // Update energyType store section
      updateFormData('energyType', {
        TIPO_MERCATO: marketType,
        includesGreenOptions: includeGreenOptions || false,
        AZIONE: actionType,
      });
    }
  }, [marketType, includeGreenOptions, actionType, updateFormData]);
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-6">
        ...
        <FormField
          control={form.control}
          name="energyType.INCLUDE_GREEN_OPTIONS"
          render={({ field }) => (
